...
function MGSWT:init_adventure_listeners()
	...
	core:add_listener(
		...
		function(context)
			...
			MGSWT:debug('after is_creating_support_army check')
			if MGSWT.malakai_support_army_cqi == nil
				...
				MGSWT:debug('creating support army')
				MGSWT:add_support_army_to_malakai()
				MGSWT.is_creating_support_army = false
				MGSWT:debug('done creating support army')
			end
			MGSWT:debug('Support army CQI %d', MGSWT.malakai_support_army_cqi)
			...
			for i=1, #unit_components do
				...
				for _=1,unit_amount do
					MGSWT:debug('Granting %s to support army ', unit_key)
